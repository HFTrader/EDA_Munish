...
#ifndef _dct_buf_2d_in_H_
#define _dct_buf_2d_in_H_
...
protected:
    ...
    // buffer instances
    dct_buf_2d_in_memcore* buffer[BufferCount];
...
public:
    ~dct_buf_2d_in();
    SC_CTOR(dct_buf_2d_in) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "dct_buf_2d_in_memcore_U_";
            name += szTmp;

            buffer[i] = new dct_buf_2d_in_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "dct_buf_2d_in.trace");
            // ports
