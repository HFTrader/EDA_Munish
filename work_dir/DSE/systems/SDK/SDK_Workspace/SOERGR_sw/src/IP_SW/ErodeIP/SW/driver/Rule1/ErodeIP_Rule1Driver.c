...
#define GRAYSCALEIP_BASEADDR 0x6e800000
...
void ErodeIP_Rule1Driver_initialize(ErodeIPRule1DriverInstance *InstancePtr, XScuGic *InterruptController, unsigned long ImgIn_BaseAddr,unsigned long ImgOut_BaseAddr, unsigned short width, unsigned short height, unsigned short horizontalActiveTime, unsigned short verticalActiveTime) {
    ...
    // for pipelined implementation with Grayscale IP, initializing it as well
    localWriteReg(GRAYSCALEIP_BASEADDR + 0x14, 0xffffffff, verticalActiveTime);
    localWriteReg(GRAYSCALEIP_BASEADDR + 0x1c, 0xffffffff, horizontalActiveTime);
    localWriteReg(GRAYSCALEIP_BASEADDR, 0x00000081, 0x81);
}
