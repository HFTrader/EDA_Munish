...
#include "../SOGRER_HierIPintegration/hwsw_func.h"
...
int main()
{
	...
	EdgeDetection_funcInit(&InterruptController, VIDEO_BASEADDR, VIDEO_BASEADDR + FRAME_SIZE, 640, 480, h_ActiveTime, v_ActiveTime);
	cpu_cycles = get_cyclecount();
	printf("CPU cycles for Sobel Init: %f \n", cpu_cycles);

	init_perfcounters(1, 0);
	Erode_funcInit(&InterruptController, VIDEO_BASEADDR, VIDEO_BASEADDR + FRAME_SIZE, 640, 480, h_ActiveTime, v_ActiveTime);
	cpu_cycles = get_cyclecount();
	printf("CPU cycles for Erode Init: %f \n", cpu_cycles);

	init_perfcounters(1, 0);
	Grayscale_funcInit(&InterruptController, VIDEO_BASEADDR, VIDEO_BASEADDR + FRAME_SIZE, 640, 480, h_ActiveTime, v_ActiveTime);
	cpu_cycles = get_cyclecount();
	printf("CPU cycles for Grayscale Init: %f \n", cpu_cycles);
