...
void config_filterVDMA(int VDMA_baseAddr, int vdma_direction, unsigned long base_address)
{
	...
	{// rx : S2MM
		...
		//Xil_Out32((VDMA_baseAddr + AXI_FILTER_RX_CTRL), 0x00000003); // enable circular mode, genlock synEn  //ORIGINAL UMAIR
		Xil_Out32((VDMA_baseAddr + AXI_FILTER_RX_CTRL), 0x00011003); // Mun
...
//
	...
	{//tx : MM2S
		...
		Xil_Out32((VDMA_baseAddr + AXI_FILTER_TX_CTRL), 0x00000003); // enable circular mode, genlock synEn  //ORIGINAL UMAIR
		//Xil_Out32((VDMA_baseAddr + AXI_FILTER_TX_CTRL), 0x00011003); // Mun
...
//	XSobel_filter_InterruptGlobalEnable(&xSobelFilter);
//	XSobel_filter_InterruptEnable(&xSobelFilter, 3);
