    ...
    if (driverInit == 0) {
    	for (ip_instance_idx=0; ip_instance_idx<ERODEIP_NUM_INSTANCES; ip_instance_idx++) {
    		if (ERODEIP_INFO[ip_instance_idx].grip_rule == 1) {
    			...
    			ErodeIP_Rule1Driver_initialize(&ErodeIPRule1Driver[rule1_driver_idx], ImgIn_BaseAddr, ImgOut_BaseAddr, width, height, horizontalActiveTime, verticalActiveTime);
    			rule1_driver_idx++;
    		} 
    	}
        ip_instance_idx = 0;
