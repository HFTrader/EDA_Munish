...
void ZYNQ_ARM_CORTEX_A9_initialize() {        
    ...
    for (i=0; i<NUM_PROCS; i++) {
        if (SoCProc_elements[i].type == ZYNQ_ARM_CORTEX_A9) {
            *(volatile (SoCProc *) (SHARED_MEM_BASEADDR + num_proc_initialized*4)) = &SoCProc_elements[i];
...
/*	// interrupting this core to run data processing chain that is passed
	...
	for (i=0; i<num_proc_initialized; i++) {
	    SoCProc *elem = *(volatile (SoCProc *) (SHARED_MEM_BASEADDR = num_proc_initialized*4));
...
void Powerup() {              
	...
	Xil_Out32((u32) elem->MEMSPACE_BASEADDR+00, (u32) 0xe3e0000f);
	Xil_Out32((u32) elem->MEMSPACE_BASEADDR+04, (u32) 0xe3a01000);
	Xil_Out32((u32) elem->MEMSPACE_BASEADDR+08, (u32) 0xe5801000);
...
void mainProcess() {
	...
	processFrame(elem->MEMSPACE_BASEADDR + 0x1000000);
