...
//	 * the specific interrupt processing for the device
	...
	Status = XScuGic_Connect(&InterruptController,XPAR_FABRIC_GRAY_SCALE_TOP_0_INTERRUPT_INTR,
			(Xil_ExceptionHandler)AXI_INTERRUPT__VDMA1_S2MMIntr_Handler,
			(void *) &InterruptController);
	XScuGic_Enable(&InterruptController, XPAR_FABRIC_GRAY_SCALE_TOP_0_INTERRUPT_INTR);
...
void InitGrayscaleFilter() {
	...
	config_filterVDMA(XPAR_AXI_VDMA_1_BASEADDR, DMA_MEM_TO_DEV, 0x2000000);
	config_filterVDMA(XPAR_AXI_VDMA_1_BASEADDR, DMA_DEV_TO_MEM, 0x3000000);
}
