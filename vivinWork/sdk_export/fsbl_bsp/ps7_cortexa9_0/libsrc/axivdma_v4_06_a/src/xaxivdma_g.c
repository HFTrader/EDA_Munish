
/*******************************************************************
*
* CAUTION: This file is automatically generated by libgen.
* Version: Xilinx EDK 14.7 EDK_P.20131013
* DO NOT EDIT.
*
* Copyright (c) 1995-2012 Xilinx, Inc.  All rights reserved.

* 
* Description: Driver configuration
*
*******************************************************************/

#include "xparameters.h"
#include "xaxivdma.h"

/*
* The configuration table for devices
*/

XAxiVdma_Config XAxiVdma_ConfigTable[] =
{
	{
		XPAR_AXI_VDMA_0_DEVICE_ID,
		XPAR_AXI_VDMA_0_BASEADDR,
		XPAR_AXI_VDMA_0_NUM_FSTORES,
		XPAR_AXI_VDMA_0_INCLUDE_MM2S,
		XPAR_AXI_VDMA_0_INCLUDE_MM2S_DRE,
		XPAR_AXI_VDMA_0_M_AXI_MM2S_DATA_WIDTH,
		XPAR_AXI_VDMA_0_INCLUDE_S2MM,
		XPAR_AXI_VDMA_0_INCLUDE_S2MM_DRE,
		XPAR_AXI_VDMA_0_M_AXI_S2MM_DATA_WIDTH,
		XPAR_AXI_VDMA_0_INCLUDE_SG,
		XPAR_AXI_VDMA_0_ENABLE_VIDPRMTR_READS,
		XPAR_AXI_VDMA_0_USE_FSYNC,
		XPAR_AXI_VDMA_0_FLUSH_ON_FSYNC,
		XPAR_AXI_VDMA_0_MM2S_LINEBUFFER_DEPTH,
		XPAR_AXI_VDMA_0_S2MM_LINEBUFFER_DEPTH,
		XPAR_AXI_VDMA_0_MM2S_GENLOCK_MODE,
		XPAR_AXI_VDMA_0_S2MM_GENLOCK_MODE,
		XPAR_AXI_VDMA_0_INCLUDE_INTERNAL_GENLOCK,
		XPAR_AXI_VDMA_0_S2MM_SOF_ENABLE,
		XPAR_AXI_VDMA_0_M_AXIS_MM2S_TDATA_WIDTH,
		XPAR_AXI_VDMA_0_S_AXIS_S2MM_TDATA_WIDTH,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_1,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_5,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_6,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_7,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_9,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_13,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_14,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_INFO_15,
		XPAR_AXI_VDMA_0_ENABLE_DEBUG_ALL
	},
	{
		XPAR_FILTER_VDMA_DEVICE_ID,
		XPAR_FILTER_VDMA_BASEADDR,
		XPAR_FILTER_VDMA_NUM_FSTORES,
		XPAR_FILTER_VDMA_INCLUDE_MM2S,
		XPAR_FILTER_VDMA_INCLUDE_MM2S_DRE,
		XPAR_FILTER_VDMA_M_AXI_MM2S_DATA_WIDTH,
		XPAR_FILTER_VDMA_INCLUDE_S2MM,
		XPAR_FILTER_VDMA_INCLUDE_S2MM_DRE,
		XPAR_FILTER_VDMA_M_AXI_S2MM_DATA_WIDTH,
		XPAR_FILTER_VDMA_INCLUDE_SG,
		XPAR_FILTER_VDMA_ENABLE_VIDPRMTR_READS,
		XPAR_FILTER_VDMA_USE_FSYNC,
		XPAR_FILTER_VDMA_FLUSH_ON_FSYNC,
		XPAR_FILTER_VDMA_MM2S_LINEBUFFER_DEPTH,
		XPAR_FILTER_VDMA_S2MM_LINEBUFFER_DEPTH,
		XPAR_FILTER_VDMA_MM2S_GENLOCK_MODE,
		XPAR_FILTER_VDMA_S2MM_GENLOCK_MODE,
		XPAR_FILTER_VDMA_INCLUDE_INTERNAL_GENLOCK,
		XPAR_FILTER_VDMA_S2MM_SOF_ENABLE,
		XPAR_FILTER_VDMA_M_AXIS_MM2S_TDATA_WIDTH,
		XPAR_FILTER_VDMA_S_AXIS_S2MM_TDATA_WIDTH,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_1,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_5,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_6,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_7,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_9,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_13,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_14,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_INFO_15,
		XPAR_FILTER_VDMA_ENABLE_DEBUG_ALL
	}
};


