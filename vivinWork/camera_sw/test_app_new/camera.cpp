...
int main(int argc, char **argv)
{
	...
	for (;;) {
		...
		else if(hw_flag == 1)
		{
			...
			//if(clock() - tmr_hw_loop < 0)
			//printf("clock() - tmr_hw_loop = %d\n", (int)(clock() - tmr_hw_loop));
			//printf("%f, %f \n", clock() - tmr_hw_loop, sum_tmr_hw_loop);
			sum_tmr_hw_loop += clock() - tmr_hw_loop;
			tmr_hw_loop = clock();
			tmr_hw_total = clock();
	...
	//		copy_rgb24todev(rgb, IMG_BASEADDR);
			...
			tmr_hw_copy_dev2mem = clock();
	//		copy_dev2mem(sobelOut->data.ptr, FILTER_OUT_BASEADDR);
			sum_tmr_hw_copy_dev2mem += clock() - tmr_hw_copy_dev2mem;

			sum_tmr_hw_total += clock() - tmr_hw_total;
			// show result (4 channel)
			cvShowImage("window", sobelOut);
			
			tmr_hw_release = clock();

			cvReleaseMat(&sobelOut);

			sum_tmr_hw_release += clock() - tmr_hw_release;
			hwCount++;
		}
